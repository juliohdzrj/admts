...
$resSearchDir=$reg_dir->regActualLocalidad($_GET["valR"]);
$row1=$resSearchDir->fetch(PDO::FETCH_ASSOC);

//print_r($row1);
//exit;
$resLocalidades=$reg_dir->allLocalidades();

//print_r($_GET);
// set page headers
$page_title = "Cambiar localidad - ".$row1["Registro"];
...
$selectLocalidadOk="";
while($row2=$resLocalidades->fetch(PDO::FETCH_ASSOC)){
	//extract($row2);
	//print_r($row2);
	if ($row2["id_localidades"]==$row1["id_localidades"]){
		$selectLocalidadOk.="<option value=\"".$row2["id_localidades"]."\" selected>".utf8_encode($row2["nombre_localidad"])."</option>";		
		}else{
			$selectLocalidadOk.="<option value=\"".$row2["id_localidades"]."\">".utf8_encode($row2["nombre_localidad"])."</option>";
			}
	
}	
echo ("<select class=\"selectpicker valedoLoc".$_GET["valR"]."\" onchange=\"changeLocalidadMedic('".$_GET["valR"]."')\">");
echo ("<option value=\"0\" selected>Selecciona localidad</option>");
echo ($selectLocalidadOk);
echo ("</select>");
