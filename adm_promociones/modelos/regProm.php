...
class regProm{
	...
	private $table_name2="encabezados";
	private $table_name3="localidades";
	private $table_name4="localidades_asignadas_promo";
	private $timestamp;
	...
	function updataXid($idpro){
		...
		function updateFileCupon(){
			$this->getTimestamp();
		$query2 = "UPDATE `".$this->table_name."` SET cupon_img=?, tamanio_cupon_img=?, tipo_cupon_img=?, nombre_cupon_img=?, tamanio_unidad_cupon_img=?, fecha_act=? WHERE id_cp_pdf_promo=?;";
		$stmt = $this->conn->prepare($query2);
		$stmt->bindParam(1, $this->content);
		$stmt->bindParam(2, $this->size);
		$stmt->bindParam(3, $this->type);
		...
		function getFimgpon($idpro2){
			$query3 = "SELECT cupon_img, tipo_cupon_img, nombre_cupon_img FROM ".$this->table_name." WHERE id_cp_pdf_promo=".$idpro2.";";
			$stmt = $this->conn->prepare($query3);		   
			if($stmt->execute()){
				return $stmt;
        	}else{
		...
		function getFpdf($idpro3){
			$query4 = "SELECT cupon_pdf, tipo_pdf, nombre_archivo_pdf, nombre_promo FROM ".$this->table_name." WHERE id_cp_pdf_promo=".$idpro3.";";
		...
		function getdataProXid($idpro4){
			...
			$query4 = "SELECT id_localidades, nombre_localidad FROM ".$this->table_name3.";";
			$stmt = $this->conn->prepare($query4);		   
			if($stmt->execute()){
				return $stmt;
        	}else{
	...
	function getProXtipo($tipoPromo2){
		//return $tipoPromo;
		$query9 = "SELECT id_cp_pdf_promo, cp_img, fecha_publicacion, fecha_caducidad, nombre_promo FROM ".$this->table_name." WHERE tipo_promo_fk=".$tipoPromo2." AND act_susp=1 ORDER BY posicion ASC;";
    ...
    function getPromoXlocalidad($tipoPromo2,$idlocalidad){
        //return $tipoPromo;
        //$query9 = "SELECT id_cp_pdf_promo, cp_img, fecha_publicacion, fecha_caducidad, nombre_promo FROM ".$this->table_name." WHERE tipo_promo_fk=".$tipoPromo2." AND act_susp=1 ORDER BY posicion ASC;";
        $query9="SELECT t1.id_cp_pdf_promo, t1.cp_img, t1.fecha_publicacion, t1.fecha_caducidad, t1.nombre_promo 
FROM ".$this->table_name." AS t1
RIGHT JOIN ".$this->table_name4." AS t2 ON t1.id_cp_pdf_promo=t2.id_cp_pdf_promo AND t2.id_localidades=".$idlocalidad."
WHERE t1.tipo_promo_fk=".$tipoPromo2." AND t1.act_susp=1 ORDER BY posicion ASC;";
        $stmt = $this->conn->prepare($query9);
        if($stmt->execute()){
            return $stmt;
        }else{
    ...
    function getProXtipoLocalidad($tipoPromo2){
        //return $tipoPromo;
        //$query9 = "SELECT id_cp_pdf_promo.t1, tipo_promo_fk.t1 FROM ".$this->table_name." WHERE tipo_promo_fk=".$tipoPromo2." AND act_susp=1 ORDER BY posicion ASC;";
        $query9="SELECT t1.id_cp_pdf_promo, t2.id_localidades, t3.nombre_localidad  
FROM ".$this->table_name." AS t1
RIGHT JOIN ".$this->table_name4." AS t2 ON t1.id_cp_pdf_promo=t2.id_cp_pdf_promo
RIGHT JOIN ".$this->table_name3." AS t3 ON t2.id_localidades=t3.id_localidades
WHERE t1.tipo_promo_fk=$tipoPromo2 AND t1.act_susp=1
ORDER BY t3.nombre_localidad ASC;";
        $stmt = $this->conn->prepare($query9);
        if($stmt->execute()){
            return $stmt;
        }else{
	...
	function getListaCatalogoTipo(){
		$query6 = "SELECT DISTINCT `tipo_catalogo` FROM ".$this->table_name2." ORDER BY tipo_catalogo ASC;";
			$stmt = $this->conn->prepare($query6);		   
			if($stmt->execute()){
				return $stmt;
        	}else{
	...
	function getHeadersList($tipocat){
		$query6 = "SELECT * FROM ".$this->table_name2." WHERE tipo_catalogo={$tipocat} ORDER BY mes ASC;";
			$stmt = $this->conn->prepare($query6);		   
			if($stmt->execute()){
				return $stmt;
        	}else{
	...
	function upImgHeaders(){
		$this->getTimestamp();
		$query2 = "UPDATE `".$this->table_name2."` SET encabezado_jpg=?, tamanio_himg=?, tipo_himg=?, nombre_himg=?, tamanio_unidad_himg=?, fecha_update_h=? WHERE idencabezado=?;";
		$stmt = $this->conn->prepare($query2);
		$stmt->bindParam(1, $this->content);
		$stmt->bindParam(2, $this->size);
		$stmt->bindParam(3, $this->type);
		...
		if($stmt->execute()){
            ...
            $errorQuery=$stmt->errorInfo();
            return "Error:UPDATE_HEADER_CAT_".$errorQuery[2];
        }			
	...
	function getHimg3($idHeader){
		$query6 = "SELECT encabezado_jpg, tipo_himg, nombre_himg FROM ".$this->table_name2." WHERE idencabezado={$idHeader};";
			$stmt = $this->conn->prepare($query6);		   
			if($stmt->execute()){
				return $stmt;
        	}else{
            $errorQuery=$stmt->errorInfo();
            return "Error:SELECT_HEADER_CAT_".$errorQuery[2];
        	}
