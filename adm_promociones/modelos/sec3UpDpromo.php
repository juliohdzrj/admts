...
$phpFileUploadErrors = array(
	...
	if($_FILES["archivo3"]["error"]!=0){
		echo ($phpFileUploadErrors[$_FILES["archivo3"]["error"]]." Error:".$_FILES["archivo3"]["error"]);
		exit;
		}
$archivosPermitidos=Array('application/pdf');
if(!in_array($_FILES["archivo3"]["type"],$archivosPermitidos)){
...
$updatePdf=$insertarFile->upPdfpromo();
	echo ($updatePdf);
if($updatePdf!="true"){
	echo ($updatePdf);
	}else{
	echo ("Se almaceno su archivo: <br> <a href=\"../vista/consultaF5.php?1perAwtXyzdd22ret56=".$_SESSION["ficha"]."\" target=\"_blank\">".$nombre_archivo."</a>");
	}
exit;		
