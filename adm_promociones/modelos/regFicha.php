...
class regFichaSMB{
	...
	private $table_name8 = "localidades_asignadas_promo";
	private $timestamp;
	...
	function valDatosFicha2(){
        ...
        WHERE idcp_pdf_ficha=?;";
                ...
                $stmt->execute();
                return $stmt->rowCount() . " records UPDATED";
            }
...
return $query_2."<br>".$e->getMessage();
}
...
WHERE e.id_categoria=".$idCatFicha2." AND c.id_localidades=".$idLoc." AND e.act_susp=1 ORDER BY e.posicion ASC;";
		...
		function getFpdf($idFicha3){
			$query4 = "SELECT idcp_pdf_ficha, id_categoria, id_tipo_promo, cp_img, bk_img, promo_pdf, suc_pdf, titulo_promo, descripcion, promocion_texto, socio, sucursales, horarios, telefono, pagina_web, facebook, twitter, email, colorpdf, tipoplan FROM ".$this->table_name2." WHERE idcp_pdf_ficha=".$idFicha3.";";
...
ORDER BY c.`nombre_localidad` ASC";
    ...
    function getLocAsigProm($idfichaPromGet){
        $query4 = "SELECT e.`id_localidades_asignadas_promo`, e.`id_cp_pdf_promo`, e.`id_localidades`, c.`nombre_localidad` 
FROM `".$this->table_name8."` AS e 
LEFT JOIN `".$this->table_name5."` AS c
ON e.`id_localidades` = c.`id_localidades`
WHERE e.`id_cp_pdf_promo`=".$idfichaPromGet."
ORDER BY c.`nombre_localidad` ASC";
        $stmt = $this->conn->prepare($query4);
        if($stmt->execute()){
            return $stmt;
            ...
            $errorQuery=$stmt->errorInfo();
            return "Error:GETDATA_LOCALIDADES_ASIGNADAS_PROM".$errorQuery[2];
        }
    }
    ...
    function delLocalidadAsigPromo($locIds,$idcppdfpromo){

        $query4 = "DELETE FROM `".$this->table_name8."` WHERE `id_localidades_asignadas_promo` IN (".$locIds.") AND id_cp_pdf_promo=".$idcppdfpromo.";";
        $stmt = $this->conn->prepare($query4);
        if($stmt->execute()){
            return "true";
        }else{
            $errorQuery=$stmt->errorInfo();
            return "Error:DELETE_LOCALIDADES_ASIGNADAS_PROMO".$errorQuery[2];
        }
...
WHERE e.id_localidades={$id_localidad} AND c.id_tipo_promo={$id_tipo_promo} AND c.act_susp=1 ORDER BY g.nombre_categoria ASC";
    ...
    function insertLocalidadAsigPromo($idpromo,$idLocalidadpromo){
        $this->getTimestamp();
        $query = "INSERT INTO
                    " . $this->table_name8 . "
                SET
                    id_cp_pdf_promo = ?, id_localidades = ?;";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $idpromo);
        $stmt->bindParam(2, $idLocalidadpromo);
        if($stmt->execute()){
            //$last_id = $this->conn->lastInsertId();
            return "Localidad asignada ".$this->timestamp;
        }else{
            $errorQuery=$stmt->errorInfo();
            return "Error:INSERT_LOCALIDAD_ASIGNADA_PROMO ".$errorQuery[2];
        }
    }
