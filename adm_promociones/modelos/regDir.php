...
class regDirSMB{
    ...
    function allDirXid($idSocioMedic){
        $query = "SELECT No,Registro,Especialidad,Subespecialidad,Empresa,Nombre,Estado,imgficha FROM  `".$this->table_name."` WHERE No=".$idSocioMedic."";
        $stmt = $this->conn->prepare($query);
        if($stmt->execute()){
            return $stmt;
        }else{
	...
	function allEspecialidades(){
		...
		$query = "SELECT id_especialidad, nombre_especialidad, estado FROM  `".$this->table_name1."` ORDER BY nombre_especialidad ASC";
			$stmt = $this->conn->prepare($query);		   
			if($stmt->execute()){
				return $stmt;
        	}else{
    ...
    function especialidadXid($idespecialidad){

        $query = "SELECT id_especialidad, nombre_especialidad, estado, imagenesp from  `".$this->table_name1."` WHERE id_especialidad=".$idespecialidad."";
        $stmt = $this->conn->prepare($query);
        if($stmt->execute()){
            return $stmt;
        }else{
            $errorQuery=$stmt->errorInfo();
            return "Error:GETDATA_ESPECIALIDADxID".$errorQuery[2];
        }

    }

	function updespecialidadesxid($idupdregistro,$nameEspecialidadnew){
        $this->getTimestamp();
	    //return $this->timestamp;
        $query8 = "UPDATE `".$this->table_name1."` SET nombre_especialidad=?, fecha_update=?  WHERE id_especialidad=?;";
        $stmt = $this->conn->prepare($query8);
        $stmt->bindParam(1, $nameEspecialidadnew);
        $stmt->bindParam(2, $this->timestamp);
        $stmt->bindParam(3, $idupdregistro);

        if($stmt->execute()){
            return array(true,"Registro actualizado: ".$this->timestamp);
        }else{
            $errorQuery=$stmt->errorInfo();
            return array(false,"Error:UPDATE_nameESPECIALIDAD-dirmedic".$errorQuery[2]);
        }
    }

    function regEspeMedicNew($nameNewEspe){
        $this->getTimestamp();
        $query="INSERT INTO
                    ".$this->table_name1."
                SET
                nombre_especialidad=?, fecha_registro=?";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $nameNewEspe);
        $stmt->bindParam(2, $this->timestamp);
        if($stmt->execute()){
            return "Especialidad registrada ".$this->timestamp;
        }else{
            $errorQuery=$stmt->errorInfo();
            return "Error:registroMedicEspecial".$errorQuery[2];
        }
    }
    ...
    function getFichaMedicaEspecialidad($idpromedic2){
        $query = "SELECT t1.No, t1.Especialidad, t1.imgficha, t1.Nombre, t1.Sucursal, t1.Direccion, t1.Delegacion, t1.Estado, t1.Telefonos, t1.Mail, t1.Horarios_atencion, t1.Paginaweb, t1.Promocion_human, t1.id_especialidad, t2.nombre_localidad 
FROM `".$this->table_name."` AS t1
LEFT JOIN `".$this->table_name2."` AS t2 ON t1.id_localidades=t2.id_localidades 
WHERE t1.No='".utf8_decode($idpromedic2)."'";
        $stmt = $this->conn->prepare($query);
        if($stmt->execute()){
            return $stmt;
        }else{
    ...
    function changeEdoEspecialidadMedic($edoEspeMedic,$valedo){
        $this->getTimestamp();
	    $query8 = "UPDATE `".$this->table_name1."` SET estado=? WHERE id_especialidad=?;";
        $stmt = $this->conn->prepare($query8);
        $stmt->bindParam(1, $valedo);
        $stmt->bindParam(2, $edoEspeMedic);
        if($stmt->execute()){
            return "Registro actualizado ".$this->timestamp;
        }else{
            $errorQuery=$stmt->errorInfo();
            return "Error:UPDATE_ESPECIALIDAD".$errorQuery[2];
        }
...
Registro = ?, Tipo_Red = ?, Especialidad = ?, Subespecialidad = ?, imagen = ?, Tipo = ?, Empresa = ?, Nombre = ?, Sucursal = ?, Direccion = ?, Delegacion = ?, Estado = ?, Telefonos = ?, Mail = ?, Horarios_atencion = ?, Paginaweb = ?, Estudios = ?, Promocion_human = ?, Observaciones = ?, imgficha = ?, fecha_reg = ? ";			
    ...
    function updatefichaimg($idfichaact,$namefileimg){
        //return $namefileimg;
        $this->getTimestamp();
        $query1 = "UPDATE `".$this->table_name."` SET imgficha=?, fecha_update=? WHERE No=?;";

        $stmt = $this->conn->prepare($query1);
        $stmt->bindParam(1, utf8_decode($namefileimg));
        $stmt->bindParam(2, $this->timestamp);
        $stmt->bindParam(3, $idfichaact);

        if($stmt->execute()){
            return "Datos actualizados ".$this->timestamp;
        }else{
            $errorQuery=$stmt->errorInfo();
            return "Error:updateimagenNewFicha".$errorQuery[2];
        }
    }

    function updateespimg($idespact,$namefileimg2){
        //return $namefileimg;
        $this->getTimestamp();
        $query1 = "UPDATE `".$this->table_name1."` SET imagenesp=?, fecha_update=? WHERE id_especialidad=?;";
        $stmt = $this->conn->prepare($query1);
        $stmt->bindParam(1, utf8_decode($namefileimg2));
        $stmt->bindParam(2, $this->timestamp);
        $stmt->bindParam(3, $idespact);
        if($stmt->execute()){
            return "Datos actualizados ".$this->timestamp;
        }else{
            $errorQuery=$stmt->errorInfo();
            return "Error:updateimagenesp".$errorQuery[2];
        }
    }
