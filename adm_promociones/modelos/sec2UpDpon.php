...
$phpFileUploadErrors = array(
	...
	if($_FILES["archivoCupon"]["error"]!=0){
		echo ($phpFileUploadErrors[$_FILES["archivoCupon"]["error"]]." Error:".$_FILES["archivoCupon"]["error"]);
		exit;
		}
$archivosPermitidos=Array('image/jpeg');
if(!in_array($_FILES["archivoCupon"]["type"],$archivosPermitidos)){
	echo ("Solo se permite imagen jpeg (foto)");
	exit;
	}

$archivo = $_FILES["archivoCupon"]["tmp_name"];
$tamanio = $_FILES["archivoCupon"]["size"];
$tamanioProcesado=false;
$tipo = $_FILES["archivoCupon"]["type"];
$nombre_archivo = $_FILES["archivoCupon"]["name"];


set_time_limit(0);	  
$folder = "img/";
$file = basename( $_FILES['archivoCupon']['name']);
$full_path = $folder.$file;

if(move_uploaded_file($_FILES['archivoCupon']['tmp_name'], $full_path)) { 
//echo "Carga exitosa";
$img_url= $full_path;
/*************************************
 * 
...
$updateImg=$insertarFile->updateFileCupon();
if($updateImg==="false"){
	echo ($updateImg);
	}else{
	echo ("Se almaceno su archivo: <br> <a href=\"../vista/consultaFpon.php?1perAwtXyzdd22ret56=".$_SESSION["prom"]."\" target=\"_blank\">".$nombre_archivo."</a>");
	}
unlink($img_url);
unlink($nuevaImgName);	
exit;		
